...
def dashboard():
    ...
    active_complaints_count = len(current_complaints)
    
    total_complaints_count = len(complaint_history)
    
    resolved_complaints_count = len([complaint for complaint in complaint_history if complaint.status == 'Resolved'])

    # Pass the counts and lists to the template
...
def admin_dashboard():
    ...
    users_count = User.query.count()  # Count total users
    complaints_count = Complaint.query.count()  # Count total complaints
    active_complaints_count = Complaint.query.filter(Complaint.status.in_(['Pending', 'In Progress'])).count()  # Count active complaints
...
def admin_database():
    ...
    # Add an order_by clause to each query to avoid the MSSQL error
    users = User.query.order_by(User.id).paginate(page=page, per_page=per_page)
    complaints = Complaint.query.order_by(Complaint.id).paginate(page=page, per_page=per_page)
    notifications = Notification.query.order_by(Notification.id).paginate(page=page, per_page=per_page)
...
def add_user():
    ...
    if form.validate_on_submit():
        ...
        user = User(
            ...
            password_hash=hashed_password  # Use the hashed password instead
        )
        
        db.session.add(user)
        db.session.commit()
        flash('User added successfully!', 'success')
        return redirect(url_for('admin.admin_database'))  # Assuming this redirects to user list or dashboard
